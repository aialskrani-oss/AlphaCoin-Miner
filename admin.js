...
import {
  ref, get, set, update, remove, onValue, runTransaction, query, orderByChild, limitToLast
...
function renderUsersTable() {
  ...
  tbody.innerHTML = slice.map(([uid, u]) => {
    ...
    return `<tr>
      <td><span style="font-weight:600">${escHtml(u.username||"—")}</span>${u.isBanned?`<span class="banned-badge">محظور</span>`:""}<br/>
      <span style="font-size:.68rem;color:var(--text3)">${escHtml(u.email||"")}</span></td>
