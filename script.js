...
import {
  ref, get, set, update, onValue, runTransaction, orderByChild, limitToLast, query
...
const POWER_UPGRADES = [
  ...
  { level:2, power:2,  label:"2 α/ساعة",  cost:1000 },
  { level:3, power:5,  label:"5 α/ساعة",  cost:3000 },
  { level:4, power:10, label:"10 α/ساعة", cost:7000 },
  { level:5, power:20, label:"20 α/ساعة", cost:15000 },
];
const DURATION_UPGRADES = [
  { level:1, hours:1,  label:"1 ساعة",  cost:0 },
  { level:2, hours:4,  label:"4 ساعات", cost:1500 },
  { level:3, hours:8,  label:"8 ساعات", cost:4000 },
  { level:4, hours:24, label:"24 ساعة", cost:10000 },
];
...
async function applyReferralCode(code) {
  ...
  if (refData.usedBy && refData.usedBy[currentUser.uid]) return "استخدمت هذا الكود من قبل";
...
function startMiningTimer() {
  stopMiningTimer();
  miningTimer = setInterval(updateMiningUI, 500);
}
...
window.buyUpgrade = async (type, level) => {
  ...
  try {
    const txResult = await runTransaction(ref(db, `users/${currentUser.uid}`), user => {
      if (!user) return user;
      if ((user.balance || 0) < upg.cost) return;
...
document.getElementById("coupon-btn")?.addEventListener("click", async () => {
  ...
  if (!code) { if (msg) { msg.textContent="أدخل الكود"; msg.className="coupon-msg err"; } return; }
...
function spawnRewardBurst(amount) {
  ...
  for (let i=0; i<12; i++) {
    ...
    el.textContent = "+α";
    el.style.cssText = `left:${20+Math.random()*60}%;top:${20+Math.random()*60}%;animation-delay:${Math.random()*0.5}s;`;
